...
If you ask for `"biomass"`, the output table will also include TOTAL_NO (abundance) and NMEAS (number of fish measured). 

If you include length constraints, the output table will include additional columns NMEAS (number of fish measured), NMEAS_SIZE (number measured in the size range), and NSIZE (total number in the size range, which is probably what you want for abundance, not TOTAL_NO). 
...
If a haul had no fish, it will appear in the output dataset (with TOTAL_NO=0). If a haul had fish, but no fish were measured, there will be a TOTAL_NO>0, NMEAS will be 0, and there will be a single length/mass entry for that haul, which will be the average value used as a substitute.
