...
#  To update, replace end date with max date on website + 1 day
...
test=filter(HAULSTANDARD, YEAR>=1990)
...
#length data
download.file(url="https://oceanview.pfeg.noaa.gov/erddap/tabledap/FED_Rockfish_Length.csv?cruise%2Chaul_no%2Cstd_length%2Cspecies_code%2Cmaturity&time%3E=1990-05-13&time%3C=2025-06-20T02%3A44%3A00Z",
...
LENGTH_ERDDAP=LENGTH_ERDDAP %>% rename(SPECIES=SPECIES_CODE) %>% select(CRUISE, HAUL_NO, SPECIES, MATURITY, STD_LENGTH)
